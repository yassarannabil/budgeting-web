...
const SuggestBudgetsInputSchema = z.object({
  historicalTransactions: z.string().describe('A JSON string containing the historical transaction data. Each transaction should have category and amount fields.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a personal finance advisor. Analyze the user's historical spending data and suggest reasonable monthly budget amounts for each category.

  Historical Transactions (JSON string):
  {{historicalTransactions}}

  Provide the output as a JSON object where keys are spending categories and values are the suggested budget amounts for each category.  Do not include any text other than the JSON object.
  `,
});
